...
function calculate() {
	...
	da = [...L.da];
	accel = d2a(a, da);
	L.a[0] += dt * (da[0] + 0.5 * dt * accel[0]);
	L.a[1] += dt * (da[1] + 0.5 * dt * accel[1]);
	for (let i = 0; i < 100; i++) {
		let newAccel = d2a(L.a, L.da);
		L.da[0] = da[0] + (accel[0] + newAccel[0]) * 0.5 * dt;
		L.da[1] = da[1] + (accel[1] + newAccel[1]) * 0.5 * dt;
	}
	...
	jerk = d3a(a, da);
	jounce = d4a(a, da);
	T.a[0] += dt * (da[0] + dt * 0.5 * (accel[0] + dt / 3 * (jerk[0] + jounce[0] * dt / 4)));
	T.a[1] += dt * (da[1] + dt * 0.5 * (accel[1] + dt / 3 * (jerk[1] + jounce[1] * dt / 4)));
	for (let i = 0; i < 4; i++) {
		let newJounce = d4a(T.a, T.da);
		T.da[0] = da[0] + dt * (accel[0] + dt * 0.5 * (jerk[0] + (jounce[0] + newJounce[0]) * dt / 6));
		T.da[1] = da[1] + dt * (accel[1] + dt * 0.5 * (jerk[1] + (jounce[1] + newJounce[1]) * dt / 6));
	}
